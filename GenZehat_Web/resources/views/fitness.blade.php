...
<head>
    ...
    /* === TAMBAHAN BARU: WARNA STATUS (HIJAU & MERAH) === */
    
    /* 1. Status Completed (Selesai) -> HIJAU */
    .day-box.completed {
        border: 2px solid #2ecc71; /* Border kotak jadi hijau */
    }
    .day-box.completed .day-status i {
        color: #2ecc71; /* Ikon lingkaran jadi hijau */
        filter: drop-shadow(0 0 3px rgba(46, 204, 113, 0.4)); /* Efek bersinar dikit */
        transform: scale(1.2); /* Ikon sedikit membesar */
        transition: all 0.3s ease;
    }

    /* 2. Status Missed (Terlewat) -> MERAH */
    .day-box.missed {
        border: 2px solid #e74c3c; /* Border kotak jadi merah */
    }
    .day-box.missed .day-status i {
        color: #e74c3c; /* Ikon lingkaran jadi merah */
...
@else
   ...
   <script>
    // Pastikan variabel didefinisikan dengan aman (menggunakan null coalescing ??)
    window.currentUser = @json(Auth::user()->username ?? null);
    
    // Jika data null, berikan array kosong []
    window.dbData = @json($dailyProgress ?? []); 
    window.userHistory = @json($histories ?? []);
</script>
